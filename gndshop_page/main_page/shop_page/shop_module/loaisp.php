...
<?php
	...
	$idloaisp = $_GET['loaispnav'];
	$sql = "SELECT * FROM sp WHERE ID_LoaiSp='".$idloaisp."' LIMIT $begin,2";
	$query = mysqli_query($mysqli,$sql);
 ?>

<div class="row">
	<?php 
		$sql_loaisp_title= "SELECT * FROM loaisp WHERE loaisp.ID_LoaiSp='".$idloaisp."'";
		$query_title= mysqli_query($mysqli,$sql_loaisp_title);
		$take_title = mysqli_fetch_array($query_title);
	?>
	<h5 class="ml-3"> SẢN PHẨM CỦA LOẠI SẢN PHẨM: <?php echo $take_title['Ten_LoaiSp'] ?></h5>
	<!-- filter-->
...
<?php
	$query_page =mysqli_query($mysqli,"SELECT * FROM sp WHERE ID_LoaiSp='".$idloaisp."'");
    ...
    <div class="ml-auto">
        ...
        ?>
            <li class="page-item <?php if($i==$page_current){echo 'active';}else{ echo '';} ?>">
                <a class="page-link" href="index.php?action2=shopmenu&dmspnav=<?php echo $iddm ?>&loaispnav=<?php echo $idloaisp ?>&page=<?php echo $i ?>"><?php echo $i ?></a>
            </li>
